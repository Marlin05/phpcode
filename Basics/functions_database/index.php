...
*/
//добавить символ в поле text c id > 4
...
// выводит все записи
/*$res = mysqli_query($db, "SELECT * FROM gb"); 
$data = mysqli_fetch_all($res MSQLI_BOTH); // нум и асс
// MSQLI_ASSOS - асс.rr
// MSQLI_NUM - нум.arr
...
*/
$str = "d'Artanian";
$str = mysqli_real_escape_string($db, $str); // экранирует '
/*$query = "INSERT INTO gb (name, text) VALUES ('$str', 'Имя с апострофом')";
mysqli_query($db, $query) or die(mysqli_error($db));
*/
// выбранные поля для сортировки по id в обратном порядке
$res = mysqli_query($db, "SELECT id, name, text, date FROM gb ORDER BY id DESC ");
 //echo mysqli_num_rows($res); // сколько рядов было выбрано
$data = mysqli_fetch_all($res, MYSQLI_ASSOC);


// 2 вар. как пол.доступ к данным и сформ.arr
/*$data2 = [];
